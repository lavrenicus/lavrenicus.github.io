...
/**
 * CorridorGenerator handles the procedural creation of sci-fi corridor segments.
 * It replaces simple planes with a layered architectural approach:
 ...
 * modeled after Jetpack Joyride LOCATIONS): _addStructure gives every
 ...
 * Sync rule: keep src/lib/corridor-gen.ts identical in layout/behavior.
 */
const TEXTURE_BASE = "/runner/assets/corridor/";

export class CorridorGenerator {
    constructor(config) {
        this.CH = config.CH || 4;   // Height
        this.CW = config.CW || 3;   // Half-width/depth
        this.SEG = config.SEG || 2; // Segment length
        this._onError = config.onError;
        // Max anisotropy from the renderer: kills the moire shimmer on the
        // oblique floor/ceiling (owner's texture verdict 2026-09-24).
        this._maxAniso = config.maxAnisotropy || 1;
        this._loader = new THREE.TextureLoader();
        // Caches: one geometry per distinct shape, one material set per
        // theme key (owner order 2026-09-25).
        this._geoCache = new Map();
        this._matCache = new Map();
        // Foreground silhouette material (createForegroundSegment): FIXED
        // near-black and deliberately WITHOUT a userData.role — the
        // dissolve arms only role-tagged materials, so the shadow frame in
        // front of the runner never morphs when the location changes.
        this._silMat = new THREE.MeshStandardMaterial({
            color: 0x060608,
            metalness: 0.1,
            roughness: 0.9
        });
        this._silMat.userData.cached = true;
        // One shared tile per surface type: one tile per world unit of the face.
        this._floorTex = this._loadTile("floor-grate.png", this.SEG, this.CW * 2);
        this._ceilTex = this._loadTile("ceiling-vent.png", this.SEG, this.CW * 2);
        this._wallTex = this._loadTile("wall-tile.png", this.SEG, this.CH);
    }

    _loadTile(file, repeatX, repeatY) {
     ...
     * @param {Object} theme
     * @returns {{shell: THREE.Color, base: THREE.Color, metal: THREE.Color, accent: THREE.Color}}
     */
    themePalette(theme) {
    ...
    _geo(cacheKey, make) {
        let g = this._geoCache.get(cacheKey);
        if (!g) {
            g = make();
            g.userData.cached = true; // rebuilds must never dispose this
            this._geoCache.set(cacheKey, g);
        }
        return g;
    }
    ...
    _box(w, h, d) {
        ...
        w = q(w); h = q(h); d = q(d);
        return this._geo(`box|${w}|${h}|${d}`, () => new THREE.BoxGeometry(w, h, d));
    }

    _cyl(rTop, rBot, h, seg) {
        const q = (v) => Math.round(v * 1000) / 1000;
        rTop = q(rTop); rBot = q(rBot); h = q(h);
        return this._geo(`cyl|${rTop}|${rBot}|${h}|${seg}`, () => new THREE.CylinderGeometry(rTop, rBot, h, seg));
    }

    _sph(r, wSeg, hSeg) {
        r = Math.round(r * 1000) / 1000;
        return this._geo(`sph|${r}|${wSeg}|${hSeg}`, () => new THREE.SphereGeometry(r, wSeg, hSeg));
    }

    _tor(r, tube, rSeg, tSeg) {
        const q = (v) => Math.round(v * 1000) / 1000;
        const a = q(r); const b = q(tube);
        return this._geo(`tor|${a}|${b}|${rSeg}|${tSeg}`, () => new THREE.TorusGeometry(a, b, rSeg, tSeg));
    }
     ...
     */
    _floorSlot(idx) {
        const j = (this._hash(idx * 53 + 3) - 0.5) * 0.16;
        return {
            x: (i) => 0.55 + i * 0.34 + j,
            z: (i) => -2.18 + this._hash(idx * 37 + i) * 0.5,
        };
    }
    ...
    _materials(theme) {
        const cached = this._matCache.get(theme.key);
        ...
        // Role tags: the location-switch dissolve arms exactly these
        // materials (palette morph ON the geometry — no freeze, no post).
        ...
        this._matCache.set(theme.key, mats);
        return mats;
    }

    /**
     * Creates a detailed segment based on the current theme.
     * @param {Object} theme The current theme configuration (must carry key)
     * @param {number} offsetX The X position offset for the segment
     * @returns {THREE.Group} The constructed segment group
     */
    createDetailedSegment(theme, offsetX) {
        const group = new THREE.Group();

        // Cached resources: rebuilds allocate plain Object3Ds only.
        const mats = this._materials(theme);
        ...
        this._addBaseShell(group, mats);
        this._addBackWall(group, mats);
        this._addStructure(group, mats, idx);

        // Archetype dispatch — explicit, no substitute path.
        switch (theme.key) {
            case "experience": this._addFactory(group, mats, idx); break;
            case "home": this._addCity(group, mats, idx); break;
            case "games": this._addArcade(group, mats, idx); break;
            case "assets": this._addGallery(group, mats, idx); break;
            case "tech": this._addTerminal(group, mats, idx); break;
            case "about": this._addBlueprint(group, mats, idx); break;
            case "links": this._addStars(group, mats, idx); break;
            default:
                throw new Error(
    ...
    _addBaseShell(group, mats) {
        // Floor
        const floor = new THREE.Mesh(
            this._box(this.SEG, 0.1, this.CW * 2),
            mats.floor
        );
        ...
        // NOTE: no cross wall at segment starts — those half-depth blades
        // were the " лишние перегородки" (owner verdict): the tube stays
        // continuous; the proper back wall is added in the panel layer.
    }

    _addBackWall(group, mats) {
        // Far side back wall only (z=-CW); camera side stays open (side view).
        const sideWall = new THREE.Mesh(
            this._box(this.SEG, this.CH, 0.2),
            mats.wall
        );
    ...
    _addStructure(group, mats, idx) {
        ...
        const wallPipe = new THREE.Mesh(
            this._cyl(0.09, 0.09, this.SEG, 8),
            mats.metal
        );
    ...
    // from _addStructure — factory only keeps its approved cap on top of
    // it; floor layer below was added by owner order 2026-09-25.
    _addFactory(group, mats, idx) {
        const big = ((idx % 4) + 4) % 4 === 1;
        // Tunnel-rib cap: the column itself is the shared support arch
        // (_addStructure, same 2-segment rhythm) — cap rides on top of it.
        if (Math.abs(idx % 2) === 0) {
            const cap = new THREE.Mesh(
                this._box(0.5, 0.3, 1.0),
                mats.metal
            );
        ...
        for (const z of [-1.6, -1.05]) {
            const pipe = new THREE.Mesh(
                this._cyl(0.07, 0.07, this.SEG, 8),
                mats.base
            );
        ...
        const conduit = new THREE.Mesh(
            this._cyl(0.04, 0.04, this.SEG, 6),
            mats.accentDim
        );
        ...
        if (big) {
            const door = new THREE.Mesh(this._box(1.1, 3.05, 0.3), mats.metal);
            door.position.set(this.SEG / 2, 1.625, -2.75);
            group.add(door);
            for (const dx of [0.47, 1.53]) {
                const edge = new THREE.Mesh(this._box(0.07, 3.05, 0.07), mats.accentHot);
                edge.position.set(dx, 1.625, -2.585);
                group.add(edge);
            }
            const lintel = new THREE.Mesh(this._box(0.5, 0.1, 0.1), mats.accentHot);
            lintel.position.set(this.SEG / 2, 3.28, -2.585);
            group.add(lintel);
        }

        // ── floor layer (owner order 2026-09-25: "вещи на полу") ──
        const s = this._floorSlot(idx);
        if (Math.abs(idx % 2) !== 0) {
            // crate stack
            const crate = new THREE.Mesh(this._box(0.5, 0.45, 0.5), mats.base);
            crate.position.set(s.x(0), 0.275, s.z(0));
            group.add(crate);
            const crate2 = new THREE.Mesh(this._box(0.32, 0.28, 0.32), mats.metal);
            crate2.position.set(s.x(0) + 0.04, 0.64, s.z(0) - 0.03);
            group.add(crate2);
            const pallet = new THREE.Mesh(this._box(0.6, 0.08, 0.45), mats.base);
            pallet.position.set(s.x(1), 0.09, s.z(1));
            group.add(pallet);
        } else {
            // barrel
            const barrel = new THREE.Mesh(this._cyl(0.2, 0.2, 0.55, 10), mats.metal);
        ...
        const spoolF = new THREE.Mesh(this._cyl(0.26, 0.26, 0.06, 12), mats.base);
        spoolF.position.set(s.x(2), 0.08, s.z(2));
        group.add(spoolF);
        const spoolC = new THREE.Mesh(this._cyl(0.13, 0.13, 0.3, 10), mats.metal);
        spoolC.position.set(s.x(2), 0.26, s.z(2));
        group.add(spoolC);
        // coiled cable lying on the floor
        const coil = new THREE.Mesh(this._tor(0.18, 0.05, 8, 20), mats.base);
    ...
    _addCity(group, mats, idx) {
        const big = ((idx % 4) + 4) % 4 === 1;
        const curb = new THREE.Mesh(
            this._box(this.SEG, 0.2, 0.4),
            mats.metal
        );
        curb.position.set(this.SEG/2, 0.1, -2.75);
        group.add(curb);
        const curbLine = new THREE.Mesh(
            this._box(this.SEG, 0.04, 0.04),
            mats.accentDim
        );
        ...
        if (big) {
            ...
            const sfFrame = new THREE.Mesh(this._box(1.6, 2.0, 0.3), mats.metal);
            sfFrame.position.set(this.SEG / 2, 1.4, -2.75);
            group.add(sfFrame);
            const sfGlass = new THREE.Mesh(this._box(1.4, 1.8, 0.06), mats.accentHot);
            sfGlass.position.set(this.SEG / 2, 1.4, -2.6);
            group.add(sfGlass);
            const mullion = new THREE.Mesh(this._box(0.07, 1.8, 0.07), mats.metal);
            ...
            // Neon signs above the windows (odd: vertical, even:
            // horizontal; vertical lowered to y=3.25 to clear the beam).
            if (Math.abs(idx % 2) === 0) {
                const sign = new THREE.Mesh(
                    this._box(0.7, 0.3, 0.05),
                    mats.accentHot
                );
        ...
        if (Math.abs(idx % 2) !== 0) {
            // dumpster + lid
            const dump = new THREE.Mesh(this._box(0.55, 0.45, 0.45), mats.metal);
            dump.position.set(s.x(0), 0.275, s.z(0));
            group.add(dump);
            const lid = new THREE.Mesh(this._box(0.6, 0.07, 0.5), mats.base);
            lid.position.set(s.x(0), 0.54, s.z(0));
            group.add(lid);
        } else {
            // cardboard boxes
            const box1 = new THREE.Mesh(this._box(0.35, 0.3, 0.35), mats.base);
            box1.position.set(s.x(0), 0.2, s.z(0));
            group.add(box1);
            const box2 = new THREE.Mesh(this._box(0.26, 0.22, 0.26), mats.base);
            box2.position.set(s.x(0) + 0.1, 0.46, s.z(0) - 0.05);
            group.add(box2);
        }
        // flattened cardboard on the floor
        const flat = new THREE.Mesh(this._box(0.3, 0.04, 0.24), mats.base);
        flat.position.set(s.x(1), 0.07, s.z(1));
        group.add(flat);
        // trash bags (squashed spheres)
        const bag1 = new THREE.Mesh(this._sph(0.15, 8, 6), mats.metal);
        bag1.scale.set(1, 0.7, 1);
        bag1.position.set(s.x(2), 0.16, s.z(2));
        group.add(bag1);
        const bag2 = new THREE.Mesh(this._sph(0.11, 8, 6), mats.base);
    ...
    _addArcade(group, mats, idx) {
        ...
        for (const z of [-1.5, -0.9]) {
            const tube = new THREE.Mesh(
                this._cyl(0.05, 0.05, this.SEG, 8),
                mats.accentDim
            );
        ...
        if (big) {
            const bezel = new THREE.Mesh(this._box(1.6, 1.9, 0.28), mats.metal);
            bezel.position.set(this.SEG / 2, 2.3, -2.76);
            group.add(bezel);
            const inner = new THREE.Mesh(this._box(1.4, 1.7, 0.06), mats.accentHot);
            inner.position.set(this.SEG / 2, 2.3, -2.6);
            group.add(inner);
            for (const dx of [0.27, 1.73]) {
                const glow = new THREE.Mesh(this._box(0.05, 1.9, 0.05), mats.accentDim);
        ...
        const coin1 = new THREE.Mesh(this._cyl(0.17, 0.17, 0.07, 14), mats.accentHot);
        coin1.position.set(s.x(0), 0.085, s.z(0));
        group.add(coin1);
        const coin2 = new THREE.Mesh(this._cyl(0.15, 0.15, 0.06, 14), mats.accentHot);
        coin2.position.set(s.x(0) + 0.05, 0.15, s.z(0) + 0.03);
        group.add(coin2);
        // game boxes leaning against each other
        const gb1 = new THREE.Mesh(this._box(0.3, 0.45, 0.12), mats.accentArt);
        gb1.position.set(s.x(1), 0.275, s.z(1));
        gb1.rotation.z = 0.12;
        group.add(gb1);
        const gb2 = new THREE.Mesh(this._box(0.26, 0.34, 0.1), mats.base);
        gb2.position.set(s.x(1) + 0.14, 0.22, s.z(1) + 0.06);
        gb2.rotation.z = -0.15;
        group.add(gb2);
        // glowing floor tiles — aligned with the pile slots (grouped look)
        for (let k = 0; k < 3; k++) {
            const tile = new THREE.Mesh(this._box(0.2, 0.03, 0.2), mats.accentDim);
    ...
    _addGallery(group, mats, idx) {
        const big = ((idx % 4) + 4) % 4 === 1;
        if (big) {
            // ── big wall object (plan: крупный объект вместо мелочи) —
            // grand painting; the pedestal exhibit of the odd segment
            // stands in FRONT of its lower half (real gallery layering) ──
            const frame = new THREE.Mesh(this._box(1.7, 1.1, 0.07), mats.metal);
            frame.position.set(this.SEG / 2, 1.45, -2.86);
            group.add(frame);
            const art = new THREE.Mesh(this._box(1.55, 0.95, 0.04), mats.accentArt);
            art.position.set(this.SEG / 2, 1.45, -2.83);
            group.add(art);
            const pedestal = new THREE.Mesh(this._box(0.5, 0.85, 0.4), mats.base);
            pedestal.position.set(this.SEG / 2, 0.425, -2.5);
            group.add(pedestal);
            const plate = new THREE.Mesh(this._box(0.56, 0.05, 0.46), mats.metal);
            plate.position.set(this.SEG / 2, 0.875, -2.5);
            group.add(plate);
            const sculpture = new THREE.Mesh(this._sph(0.17, 12, 10), mats.accentArt);
            sculpture.position.set(this.SEG / 2, 1.07, -2.5);
            group.add(sculpture);
        } else if (Math.abs(idx % 2) === 0) {
            const frame = new THREE.Mesh(
                this._box(1.2, 0.95, 0.07),
                mats.metal
            );
            frame.position.set(this.SEG/2, 1.55, -2.86);
            group.add(frame);
            const art = new THREE.Mesh(
                this._box(1.04, 0.79, 0.04),
                mats.accentArt
            );
            ...
            const pedestal = new THREE.Mesh(
                this._box(0.5, 0.85, 0.4),
                mats.base
            );
            pedestal.position.set(this.SEG/2, 0.425, -2.5);
            group.add(pedestal);
            const plate = new THREE.Mesh(
                this._box(0.56, 0.05, 0.46),
                mats.metal
            );
            plate.position.set(this.SEG/2, 0.875, -2.5);
            group.add(plate);
            const sculpture = new THREE.Mesh(
                this._sph(0.17, 12, 10),
                mats.accentArt
            );
            sculpture.position.set(this.SEG/2, 1.07, -2.5);
            group.add(sculpture);
        ...
        if (Math.abs(idx % 2) === 0) {
            ...
            const crate = new THREE.Mesh(this._box(0.5, 0.4, 0.45), mats.base);
            crate.position.set(s.x(0), 0.25, s.z(0));
            group.add(crate);
            const label = new THREE.Mesh(this._box(0.18, 0.1, 0.02), mats.accentArt);
            label.position.set(s.x(0), 0.3, s.z(0) + 0.235);
            group.add(label);
            // paint bucket
            const bucket = new THREE.Mesh(this._cyl(0.12, 0.1, 0.22, 10), mats.metal);
            ...
            for (const p of [[x0, z0], [x1, z1]]) {
                const post = new THREE.Mesh(this._cyl(0.045, 0.045, 0.5, 8), mats.metal);
                post.position.set(p[0], 0.3, p[1]);
                group.add(post);
                const base = new THREE.Mesh(this._cyl(0.13, 0.13, 0.05, 10), mats.base);
                base.position.set(p[0], 0.075, p[1]);
                group.add(base);
            }
            const dx = x1 - x0; const dz = z1 - z0;
            const rope = new THREE.Mesh(
                this._box(Math.hypot(dx, dz) + 0.1, 0.03, 0.03),
                mats.accentDim
            );
    ...
    _addTerminal(group, mats, idx) {
        for (const x of [0.4, 1.0, 1.6]) {
            const v = new THREE.Mesh(
                this._box(0.02, this.CH, 0.03),
                mats.accentDim
            );
        ...
        if (big) {
            ...
            const rack = new THREE.Mesh(this._box(0.9, 3.0, 0.3), mats.metal);
            rack.position.set(this.SEG / 2, 1.55, -2.75);
            group.add(rack);
            for (const dx of [0.58, 1.42]) {
                const rail = new THREE.Mesh(this._box(0.05, 3.0, 0.05), mats.accentDim);
                rail.position.set(dx, 1.55, -2.58);
                group.add(rail);
            }
            for (let row = 0; row < 5; row++) {
                for (let col = 0; col < 3; col++) {
                    const led = new THREE.Mesh(this._box(0.12, 0.05, 0.04), mats.accentHot);
            ...
            for (let k = 0; k < 3; k++) {
                const dot = new THREE.Mesh(
                    this._box(0.07, 0.07, 0.04),
                    mats.accentHot
                );
                const lane = (((idx + k) % 3) + 3) % 3;
                dot.position.set(
                    [0.4, 1.0, 1.6][lane],
                    0.55 + this._hash(idx * 7 + k * 13) * 3.0,
                    -2.86
                );
                group.add(dot);
            }
        ...
        const srv = new THREE.Mesh(this._box(0.45, 0.5, 0.4), mats.metal);
        srv.position.set(s.x(0), 0.3, s.z(0));
        group.add(srv);
        for (const ly of [0.2, 0.42]) {
            const led = new THREE.Mesh(this._box(0.3, 0.03, 0.02), mats.accentDim);
            led.position.set(s.x(0), ly, s.z(0) + 0.21);
            group.add(led);
        }
        // cable spool (flange + core)
        const spoolF = new THREE.Mesh(this._cyl(0.24, 0.24, 0.06, 12), mats.base);
        spoolF.position.set(s.x(1), 0.08, s.z(1));
        group.add(spoolF);
        const spoolC = new THREE.Mesh(this._cyl(0.12, 0.12, 0.28, 10), mats.metal);
        spoolC.position.set(s.x(1), 0.25, s.z(1));
        group.add(spoolC);
        // toolbox + label
        const tbox = new THREE.Mesh(this._box(0.35, 0.18, 0.2), mats.base);
        tbox.position.set(s.x(2), 0.14, s.z(2));
        group.add(tbox);
        const tlabel = new THREE.Mesh(this._box(0.2, 0.05, 0.02), mats.accentArt);
        tlabel.position.set(s.x(2), 0.17, s.z(2) + 0.11);
        group.add(tlabel);
        // cable coil lying on the floor
        const coil = new THREE.Mesh(this._tor(0.16, 0.045, 8, 20), mats.accentDim);
        coil.rotation.x = Math.PI / 2;
    ...
    _addBlueprint(group, mats, idx) {
        const big = ((idx % 4) + 4) % 4 === 1;
        for (const x of [0.5, 1.0, 1.5]) {
            const v = new THREE.Mesh(
                this._box(0.015, this.CH, 0.02),
                mats.accentDim
            );
            v.position.set(x, this.CH/2, -2.88);
            group.add(v);
        ...
        if (Math.abs(idx % 2) === 0) {
            ...
            const cornerH = new THREE.Mesh(
                this._box(0.5, 0.03, 0.03),
                mats.accentHot
            );
            cornerH.position.set(0.35, 3.45, -2.86);
            group.add(cornerH);
            const cornerV = new THREE.Mesh(
                this._box(0.03, 0.5, 0.03),
                mats.accentHot
            );
        ...
        if (Math.abs(idx % 4) === 0) {
            const edge = new THREE.Mesh(
                this._box(0.03, this.CH, 0.03),
                mats.accentHot
            );
            edge.position.set(0.05, this.CH/2, -2.88);
            group.add(edge);
            const top = new THREE.Mesh(
                this._box(0.03, 0.03, this.CW * 2 - 1),
                mats.accentDim
            );
        ...
        if (big) {
            const mk = (w, h) => new THREE.Mesh(this._box(w, h, 0.04), mats.accentDim);
            ...
            group.add(rectR);
            const crossH = new THREE.Mesh(this._box(1.8, 0.02, 0.02), mats.accentHot);
            crossH.position.set(1.0, 1.85, -2.83);
            group.add(crossH);
            const crossV = new THREE.Mesh(this._box(0.02, 1.5, 0.02), mats.accentHot);
        ...
        const rollA = new THREE.Mesh(this._cyl(0.13, 0.13, 0.55, 10), mats.base);
        rollA.rotation.z = Math.PI / 2;
        rollA.position.set(s.x(0), 0.18, s.z(0));
        group.add(rollA);
        const rollB = new THREE.Mesh(this._cyl(0.13, 0.13, 0.55, 10), mats.accentArt);
        rollB.rotation.z = Math.PI / 2;
        rollB.position.set(s.x(0), 0.44, s.z(0));
        group.add(rollB);
        const strap = new THREE.Mesh(this._box(0.07, 0.56, 0.3), mats.metal);
        strap.position.set(s.x(0), 0.31, s.z(0));
        group.add(strap);
        // stack of plan sheets
        for (let k = 0; k < 3; k++) {
            const sheet = new THREE.Mesh(
                this._box(0.4, 0.03, 0.3),
        ...
        // ruler block
        const ruler = new THREE.Mesh(this._box(0.65, 0.06, 0.1), mats.accentDim);
    ...
    _addStars(group, mats, idx) {
        ...
        if (big) {
            ...
            // pipe band (2.06..2.24). ──
            const plate = new THREE.Mesh(this._box(1.5, 1.1, 0.14), mats.base);
            ...
            for (let k = 0; k < 5; k++) {
                ...
                const y = k < 3
                    ? 1.65 + this._hash(seed + 2) * 0.25
                    : 2.38 + this._hash(seed + 2) * 0.22;
                const p = new THREE.Vector3(
                    0.4 + this._hash(seed + 1) * 1.2,
                    y,
                    -2.8
                );
                pts.push(p);
                const star = new THREE.Mesh(
                    this._sph(0.05 + this._hash(seed + 3) * 0.04, 8, 6),
                    mats.accentHot
                );
            ...
            for (let k = 0; k < 4; k++) {
                const seed = idx * 41 + k * 7;
                const p = new THREE.Vector3(
                    0.2 + this._hash(seed + 1) * 1.6,
                    0.6 + this._hash(seed + 2) * 3.0,
                    -2.85 + this._hash(seed + 4) * 0.1
                );
                const r = 0.03 + this._hash(seed + 3) * 0.04;
                pts.push(p);
                const star = new THREE.Mesh(
                    this._sph(r, 8, 6),
                    mats.accentHot
        ...
        const s = this._floorSlot(idx);
        // meteor stones
        const rock1 = new THREE.Mesh(this._sph(0.16, 7, 5), mats.metal);
        rock1.scale.set(1, 0.75, 1);
        rock1.position.set(s.x(0), 0.17, s.z(0));
        group.add(rock1);
        const rock2 = new THREE.Mesh(this._sph(0.11, 7, 5), mats.base);
        rock2.scale.set(1, 0.8, 1);
        rock2.position.set(s.x(1), 0.14, s.z(1));
        group.add(rock2);
        // glowing shards
        for (let k = 0; k < 2; k++) {
            const shard = new THREE.Mesh(this._box(0.09, 0.3, 0.09), mats.accentHot);
     ...
     * Uses the FIXED near-black _silMat (cached, NO theme role): the
     ...
     * @param {number} offsetX The X position offset for the chunk
     * @returns {THREE.Group} The foreground chunk group
     */
    createForegroundSegment(offsetX) {
        const group = new THREE.Group();
        const idx = Math.round(offsetX / 8);
        const v = this._hash(idx * 23 + 5);
        const x = 1.5 + this._hash(idx * 17 + 1) * 5;
        if (v < 0.55) {
            // floor-to-frame-edge post (spans y -2..7 — covers the frame
            // at z=+3, camera-visible band is roughly y -2..6)
            const post = new THREE.Mesh(this._box(0.6, 9, 0.6), this._silMat);
            post.position.set(x, 2.5, 3);
            group.add(post);
        } else if (v < 0.8) {
            // top pipe silhouette crossing the upper frame
            const pipe = new THREE.Mesh(this._cyl(0.22, 0.22, 8, 8), this._silMat);
            pipe.rotation.z = Math.PI / 2;
