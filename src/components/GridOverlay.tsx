...
void main() {
  ...
  uv.x -= mix(0.18, 0.85, smoothstep(0.8, 1.35, aspect));
  ...
  float line = 1.0 - smoothstep(0.0, fwidth(grid) * 1.8, grid);
  float fresnel = pow(1.0 - max(dot(normal, -ray), 0.0), 1.7);
  float alpha = line * (0.32 + fresnel * 0.68) * u_intensity;
  color = vec4(0.0, 0.50, 1.0, alpha);
}`;

export default function GridOverlay({ dimmed = false }: { dimmed?: boolean }) {
